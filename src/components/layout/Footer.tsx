...
export const Footer: React.FC = () => {
  // 管理画面のサイト設定を反映する（保存がなければ従来の構成で表示）
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex space-x-4">
              {settings.snsLinks
                .filter(sns => sns.isVisible !== false && sns.url && snsIconPaths[sns.platform])
                .map(sns => (
                  <a
                    key={sns.platform}
                    href={sns.url}
                  ...
                  >
                    <span className="sr-only">{snsPlatformLabels[sns.platform]}</span>
                    <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                      <path d={snsIconPaths[sns.platform]} />
                    </svg>
                  </a>
                ))}
            </div>
              ...
              {settings.footerLegalLinks
                ...
                .map(link => (
                  <Link
                    ...
                    href={link.href}
                    className="text-emerald-600 hover:text-white text-sm transition-colors duration-200"
                  >
                    {link.label}
                  </Link>
                ))}
