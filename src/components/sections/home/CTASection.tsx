...
export function CTASection({ ov }: CTASectionProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 見出しの背後：苔緑のラジアルグラデーションがスクロールでゆっくり明るくなる
      if (glow) {
        gsap.fromTo(
          glow,
          { opacity: 0.12 },
  ...
  return (
    <section ref={sectionRef} className="py-16 sm:py-24 md:py-32 text-white relative overflow-hidden">
      <div
        ...
        style={{
          background: 'radial-gradient(ellipse at 50% 45%, rgba(120, 148, 78, 0.55), rgba(23, 41, 10, 0.1) 45%, transparent 72%)',
        }}
      />
      <Container className="relative z-10">
        <div ref={cardRef} className="text-center glass-card-dark rounded-2xl sm:rounded-3xl p-8 sm:p-12 md:p-16 mx-4">
