...
interface ProductActionsProps {
  product: Product;
  // 管理画面の送料設定から生成した割引案内文（サーバー側で算出して渡す）。
  // 空文字なら送料割引の案内行を表示しない。
  shippingNote?: string;
}

export const ProductActions: React.FC<ProductActionsProps> = ({ product, shippingNote }) => {
      ...
      <div className="border-t pt-4 text-sm text-gray-600">
        ...
        {shippingNote && (
          <div className="flex items-center gap-2">
            ...
            <span className="whitespace-pre-line">{shippingNote}</span>
          </div>
        )}
      </div>
    </div>
