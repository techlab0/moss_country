...
export default function CheckoutPage() {
  ...
  // 銀行振込メールテンプレート（HTML版）
  const generateBankTransferEmailTemplate = (orderData: any, orderDate: string, deadline: string) => {
      ...
      <body style="font-family: 'Hiragino Sans', 'Yu Gothic', 'Meiryo', sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #f8f9fa; padding: 15px; border-left: 4px solid #dc3545; margin: 20px 0;">
            <h3 style="color: #dc3545; margin-top: 0;">🏦 お振込のご案内</h3>
            <p><strong>下記口座へ${deadline}までにお振込ください。</strong></p>
            <div style="background-color: white; padding: 15px; border-radius: 5px; margin: 10px 0;">
              <table style="width: 100%;">
                <tr><td><strong>銀行名：</strong></td><td>北海道銀行</td></tr>
                <tr><td><strong>支店名：</strong></td><td>札幌駅前支店</td></tr>
                <tr><td><strong>口座種別：</strong></td><td>普通</td></tr>
                <tr><td><strong>口座番号：</strong></td><td>1234567</td></tr>
                <tr><td><strong>口座名義：</strong></td><td>MOSS COUNTRY</td></tr>
              </table>
            </div>
            <p style="font-size: 14px; color: #666;">
              ※振込手数料はお客様のご負担となります。<br>
              ※入金確認後、商品の発送準備を開始いたします。
            </p>
          </div>
          ...
          <div style="text-align: right; margin: 20px 0;">
            <p><strong>商品合計：¥${orderData.pricing.subtotal.toLocaleString()}</strong></p>
            <p><strong>送料：¥${orderData.pricing.shipping.toLocaleString()}</strong></p>
            <p style="font-size: 18px; color: #dc3545;"><strong>お振込金額：¥${orderData.pricing.total.toLocaleString()}</strong></p>
          </div>
  ...
  // 銀行振込メールテンプレート（テキスト版）
  const generateBankTransferEmailText = (orderData: any, orderDate: string, deadline: string) => {
    const items = orderData.items.map((item: any) => 
      `・${item.name} ${item.quantity}個 ¥${(item.price * item.quantity).toLocaleString()}`
    ).join('\n');

    return `
MOSS COUNTRY ご注文確認とお振込のご案内
...
商品合計：¥${orderData.pricing.subtotal.toLocaleString()}
送料：¥${orderData.pricing.shipping.toLocaleString()}
お振込金額：¥${orderData.pricing.total.toLocaleString()}
...
────────────────────────
  ...
  // 代金引換メールテンプレート（HTML版）
  const generateCODEmailTemplate = (orderData: any, orderDate: string) => {
    const items = orderData.items.map((item: any) => 
      `<tr>
        <td style="padding: 8px; border-bottom: 1px solid #ddd;">${item.name}</td>
        <td style="padding: 8px; border-bottom: 1px solid #ddd; text-align: center;">${item.quantity}個</td>
      ...
      <body style="font-family: 'Hiragino Sans', 'Yu Gothic', 'Meiryo', sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #f8f9fa; padding: 15px; border-left: 4px solid #28a745; margin: 20px 0;">
            <h3 style="color: #28a745; margin-top: 0;">📦 代金引換でのお届け</h3>
            <p>商品は代金引換でお届けいたします。配達時に以下の金額をお支払いください。</p>
            <div style="background-color: white; padding: 15px; border-radius: 5px; margin: 10px 0; text-align: center;">
              <p style="font-size: 20px; color: #dc3545; margin: 0;">
                <strong>お支払い金額：¥${orderData.pricing.total.toLocaleString()}</strong>
              </p>
              <p style="font-size: 14px; color: #666; margin: 5px 0 0 0;">
                （商品代金 + 送料 + 代引手数料 ¥300 込み）
              </p>
            </div>
            <p style="font-size: 14px; color: #666;">
              ※代金引換手数料¥300が含まれています。<br>
              ※現金のみのお支払いとなります。お釣りが必要な場合は事前にご用意ください。
            </p>
          </div>

          <h3>📋 ご注文詳細</h3>
          <p><strong>注文日時：</strong>${orderDate}</p>
          <p><strong>支払方法：</strong>代金引換</p>
          ...
          <div style="text-align: right; margin: 20px 0;">
            <p><strong>商品合計：¥${orderData.pricing.subtotal.toLocaleString()}</strong></p>
            <p><strong>送料：¥${orderData.pricing.shipping.toLocaleString()}</strong></p>
            <p><strong>代引手数料：¥300</strong></p>
            <p style="font-size: 18px; color: #dc3545;"><strong>合計金額：¥${orderData.pricing.total.toLocaleString()}</strong></p>
          </div>
          ...
          <div style="margin-top: 30px; padding: 20px; background-color: #fff3cd; border: 1px solid #ffeaa7; border-radius: 5px;">
            <h4 style="color: #856404; margin-top: 0;">📋 配達について</h4>
            <ul style="color: #856404; margin: 0; padding-left: 20px;">
              <li>商品の準備が整い次第、発送いたします</li>
              <li>配達日時の指定がある場合は、別途ご連絡いたします</li>
              <li>不在の場合は、不在票をお入れいたします</li>
              <li>現金でのお支払いをお願いいたします</li>
            </ul>
          </div>
  ...
  // 代金引換メールテンプレート（テキスト版）
  const generateCODEmailText = (orderData: any, orderDate: string) => {
    const items = orderData.items.map((item: any) => 
      `・${item.name} ${item.quantity}個 ¥${(item.price * item.quantity).toLocaleString()}`
    ).join('\n');

    return `
MOSS COUNTRY ご注文確認（代金引換）
...
お支払い金額：¥${orderData.pricing.total.toLocaleString()}
（商品代金 + 送料 + 代引手数料 ¥300 込み）
...
送料：¥${orderData.pricing.shipping.toLocaleString()}
代引手数料：¥300
合計金額：¥${orderData.pricing.total.toLocaleString()}
