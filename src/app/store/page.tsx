...
const storeInfo = {
  ...
  access: {
    subway: 'JR函館本線「発寒中央駅」より徒歩8分',
    bus: 'JRバス「発寒11条4丁目」停留所より徒歩1分',
    car: '新千歳空港より車で約50分',
  },
};

// その日の項目群からセルの配色を決める（営業＋イベント重複日は斜め2色）。
function dayVisual(items: CalendarItem[] | undefined) {
  ...
  // 営業とイベントが重なる日は左上を緑（営業）、右下を黄（イベント）の斜め2色で示す
  if (hasOpen && hasEvent) return { bg: 'bg-gradient-to-br from-emerald-200 from-50% to-amber-200 to-50%', text: 'text-emerald-900', border: 'border-emerald-400', hasEvent: true };
  ...
  return (
    <div className="bg-amber-950/50 md:bg-amber-950/20 backdrop-blur-md p-6 rounded-3xl">
      ...
      <div className="flex flex-wrap gap-4 mb-6 text-sm">
        ...
        <div className="flex items-center">
          <div className="w-4 h-4 bg-gradient-to-br from-emerald-200 from-50% to-amber-200 to-50% border border-emerald-400 rounded mr-2"></div>
...
const serviceDetails: Record<number, string[]> = {
  ...
  1: ['オリジナルテラリウム作成可能', '制作期間：ご相談の上決定', '事前相談無料'],
  2: ['植物の植え替え', '容器のクリーニング'],
