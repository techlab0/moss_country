...
async function restoreChargeInventory(
  lineItems: StoreInventoryLine[] | undefined,
  chargeId: string,
  inventoryApplied?: StoreInventoryAppliedLine[] | null
) {
  try {
    await revertStoreSaleInventory(lineItems || [], `店頭決済の返金 ${chargeId}`, inventoryApplied ?? null);
  ...
  try {
    ...
    if (charge.status === 'paid') {
      if (charge.method === 'paypay') {
        ...
        await restoreChargeInventory(charge.lineItems, id, charge.inventoryApplied ?? null);
        // バックアップ用Googleスプレッドシート同期（await-and-swallow。Cronの保険が無いため
